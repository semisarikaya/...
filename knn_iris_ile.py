...
for k in range(1, 11):
    knn = MyKNN(k=k, distance_metric='euclidean', weighted=True)
...
best_knn = MyKNN(k=best_k, distance_metric='euclidean', weighted=True)
...
# Decision boundary arka plan
plt.contourf(xx, yy, Z, alpha=0.3, cmap=plt.cm.Set2)

# Eğitim verisi gri
plt.scatter(X_train_pca[:,0], X_train_pca[:,1], c='gray', label='Eğitim verisi', alpha=0.5, s=50, edgecolor='k')
...
plt.title("KNN - Iris (4 Özellik, PCA 2D + Decision Boundary + Legend)")
plt.xlabel("PCA 1")
plt.ylabel("PCA 2")
